...
class Update extends Module
{
    ...
    public function action_setup(): void
    {
        ...
        if (!is_user_logged_in() && $this->plugin->model->get_saved_version()) {
            return;
        }

        // Get the saved version.
        $saved_version = $this->plugin->model->get_saved_version();

        // Only run updates if the version numbers don't match.
        if (!version_compare($saved_version, $this->plugin->model->get_version(), '=')) {
            $this->run($saved_version);

            $this->plugin->model->set_saved_version($this->plugin->model->get_version());
        }
    }
